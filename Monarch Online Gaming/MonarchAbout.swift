...
struct MonarchGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://monarchog.top/get")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            MonarchEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    MonarchGameInitialView()
}
