...
protocol MonarchLoadStateRepresentable {
    var type: MonarchLoadState.StateType { get }
...
struct MonarchLoadState: Equatable, MonarchLoadStateRepresentable {
    ...
    static func idle() -> MonarchLoadState {
        MonarchLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> MonarchLoadState {
        MonarchLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> MonarchLoadState {
        MonarchLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> MonarchLoadState {
        MonarchLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> MonarchLoadState {
        MonarchLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: MonarchLoadState) -> Bool {
    ...
    static func == (lhs: MonarchLoadState, rhs: MonarchLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension MonarchLoadState: CustomStringConvertible {
